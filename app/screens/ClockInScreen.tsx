...
const ClockInScreen: React.FC = () => {
  ...
  async function loadNetworks() {
    ...
    const { data: ownedLocations } = await supabase
      .from("locations").select("*").eq("user_id", user.id);
    ...
    if (memberRoles) {
      for (const r of memberRoles) {
        const { data: locationData } = await supabase
          .from("locations").select("id, name, user_id")
          .eq("user_id", r.owner_id).single();
        if (locationData) {
          connections.push({
  ...
  async function loadActiveEntry() {
    ...
    const { data } = await supabase
      ...
      .eq("user_id", user.id).is("clock_out", null).single();
    setActiveEntry(data);
  }
  ...
  async function loadUpcomingShifts() {
    ...
    const { data: shifts } = await supabase
      .from("shifts").select("*")
      .eq("assigned_to", user.id).eq("status", "scheduled")
      .gte("shift_date", today).lte("shift_date", sevenDays)
  ...
  async function handleSubmitRequest() {
    ...
    try {
      ...
      Alert.alert(
        'Request Sent ✅',
        `Your ${requestType === 'holiday' ? 'holiday' : 'unpaid leave'} request for ${days} day${days !== 1 ? 's' : ''} has been sent to ${selectedNetwork.owner_name}.`
      );
    } catch (err: any) {
  ...
  async function handleClockIn(locationId: string) {
    ...
    try {
      ...
      const { data: todayShifts } = await supabase
        .from("shifts").select("*")
        .eq("assigned_to", user.id).eq("location_id", locationId)
        .eq("shift_date", today).eq("status", "scheduled");

      if (!todayShifts || todayShifts.length === 0) {
        Alert.alert("No Shift Scheduled", "You don't have a scheduled shift today. Are you sure you want to clock in?", [
  ...
  async function performClockIn(locationId: string, shiftId?: string) {
    try {
      const { data: locationData, error: locationError } = await supabase
        .from('locations').select('user_id').eq('id', locationId).single();
  ...
  async function handleClockOut() {
    ...
    try {
      ...
      const { error } = await supabase
        .from("time_entries").update({ clock_out: new Date().toISOString() })
  ...
  async function checkShiftAlert() {
    ...
    const { data: shift } = await supabase
      .from("shifts").select("end_time").eq("id", activeEntry.shift_id).single();
    if (!shift) return;
    const now = new Date();
    ...
    if (now > thirtyMinutesAfter) {
      Alert.alert("Still Working?", "Your shift ended over 30 minutes ago. Are you still working?", [
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.container}>
        ...
        {activeEntry ? (
          <View style={styles.statusCard}>
            ...
            <Text style={styles.statusTime}>Since {new Date(activeEntry.clock_in).toLocaleTimeString()}</Text>
            <TouchableOpacity style={[styles.clockButton, styles.clockOutButton]} onPress={handleClockOut} disabled={clockingIn}>
              {clockingIn ? <ActivityIndicator size="small" color="#fff" /> : <Text style={styles.clockButtonText}>Clock Out</Text>}
            </TouchableOpacity>
          </View>
        ) : (
          <View style={styles.statusCard}>
          ...
          networks.map((network, index) => {
            ...
            return (
              <View key={`${network.location_id}-${index}`} style={styles.card}>
                ...
                {!activeEntry && (
                  <TouchableOpacity style={[styles.clockButton, styles.clockInButton]} onPress={() => handleClockIn(network.location_id)} disabled={clockingIn}>
                    {clockingIn ? <ActivityIndicator size="small" color="#fff" /> : <Text style={styles.clockButtonText}>Clock In</Text>}
                  </TouchableOpacity>
                )}
                ...
                {/* ── Request Time Off button (non-owners only) ── */}
                {network.role !== 'owner' && (
                  <TouchableOpacity
                    style={styles.requestButton}
                    onPress={() => openRequestModal(network)}
                  >
                    <Text style={styles.requestButtonText}>🏖️ Request Time Off</Text>
                  </TouchableOpacity>
                )}
              </View>
            );
            ...
            {requestsLoading ? (
              ...
              myRequests.map(req => (
                <View key={req.id} style={styles.requestRow}>
                  ...
                  <Text style={[styles.requestRowStatus, { color: statusColor(req.status) }]}>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modal}>
            <Text style={styles.modalTitle}>🏖️ Request Time Off</Text>
            {selectedNetwork && (
              <Text style={styles.modalSubtitle}>Requesting from {selectedNetwork.owner_name}</Text>
            )}
            ...
            <View style={styles.typeRow}>
              <TouchableOpacity
                style={[styles.typeButton, requestType === 'holiday' && styles.typeButtonActive]}
                onPress={() => setRequestType('holiday')}
              >
                <Text style={[styles.typeButtonText, requestType === 'holiday' && styles.typeButtonTextActive]}>
                  🏖️ Holiday
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.typeButton, requestType === 'unpaid_leave' && styles.typeButtonActive]}
                onPress={() => setRequestType('unpaid_leave')}
              >
                <Text style={[styles.typeButtonText, requestType === 'unpaid_leave' && styles.typeButtonTextActive]}>
                  💼 Unpaid Leave
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.modalButtons}>
              ...
              <TouchableOpacity
                style={[styles.modalBtn, styles.modalBtnSubmit, submitting && { opacity: 0.6 }]}
                onPress={handleSubmitRequest}
                disabled={submitting}
              >
                {submitting
                  ? <ActivityIndicator size="small" color="#fff" />
                  : <Text style={styles.modalBtnSubmitText}>Send Request</Text>
                }
              </TouchableOpacity>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  requestRowStatus:        { fontSize: 12, fontWeight: "700", marginLeft: 8, marginTop: 2 },
