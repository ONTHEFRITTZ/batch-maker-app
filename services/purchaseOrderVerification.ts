...
export async function createPurchaseOrder(
  ...
  try {
    ...
    return {
      success: true,
      po: (await fetchPurchaseOrder(po.id)) as PurchaseOrder,
    };
  } catch (error: any) {
...
export async function verifyPurchaseOrderReceipt(
  ...
  try {
    ...
    if (po.status === "received" || po.status === "cancelled") {
      return {
        success: false,
        error: `Cannot verify receipt: PO is already ${po.status}`,
      };
    }
    ...
    for (const orderedItem of po.items) {
      ...
      if (!received) {
        ...
        lineResults.push({
          ...
          variance_cost: -(
            orderedItem.quantity_ordered * orderedItem.unit_cost
          ),
        });
      ...
      const variance =
        received.quantity_received - orderedItem.quantity_ordered;
      ...
      // Determine status
      let status: ReceiptVerificationResult["line_items"][0]["status"] =
        "matched";
    ...
    for (const received of receivedItems) {
      const wasOrdered = po.items.find(
        (item) => item.id === received.po_item_id,
      );
      if (!wasOrdered) {
        lineResults.push({
    ...
    const hasShortages = discrepancies.some(
      (d) =>
        d.discrepancy_type === "short" ||
        d.discrepancy_type === "missing" ||
        d.discrepancy_type === "backorder",
    );
    ...
    // --- Build summary ---
    const totalVarianceCost = discrepancies.reduce(
      (sum, d) => sum + d.variance_cost,
      0,
    );
    const matchedCount = lineResults.filter(
      (l) => l.status === "matched",
    ).length;
    ...
    if (discrepancies.length === 0) {
      ...
      const issues = discrepancies.map((d) => {
        switch (d.discrepancy_type) {
          case "short":
            return `${d.ingredient_name}: received ${d.quantity_received} of ${d.quantity_ordered} ${d.unit}`;
          case "missing":
            return `${d.ingredient_name}: completely missing`;
          case "extra":
            return `${d.ingredient_name}: received ${d.quantity_received} (ordered ${d.quantity_ordered})`;
          case "damaged":
            return `${d.ingredient_name}: received damaged`;
          case "expired":
            return `${d.ingredient_name}: received expired`;
          case "backorder":
            return `${d.ingredient_name}: backordered`;
        }
      });
 ...
 */
export async function getPODiscrepancySummary(
  poId: string,
): Promise<PODiscrepancy[]> {
